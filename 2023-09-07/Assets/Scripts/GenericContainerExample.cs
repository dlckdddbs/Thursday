...
public class GenericContainerExample : MonoBehaviour
{
    ...
    private GenericContainer<string> stringContainer;   //컨테이너 선언
    ...
    void Update()
    {
        ...
        {
            intContainer.Add(Random.Range(0, 100));     //0~100 랜던숫자를 컨테이너에 넣는다
            DisplayContainerItems(intContainer);        //함수를 통해서 debug.log에 보여준다
        }

        if (Input.GetKeyDown(KeyCode.Alpha2))       //키보드 1을 누르면
        {
            string randomString = "Item " + Random.Range(0, 100);       //임의의 문자열을 만들어 준다.
            stringContainer.Add(randomString);     //0~100 랜던숫자를 컨테이너에 넣는다
    ...
    private void DisplayContainerItems<T>(GenericContainer<T> container)
    {
        ...
        for(int i =0; i<item.Length; i++)           //컨테이너의 모든 값을 for 문으로 돌면서
        {
            ...
            {
                temp += "Empty / ";     //null일 경우에는 Empty 표현해준다.
            }
        }
        Debug.Log(temp);
    }
